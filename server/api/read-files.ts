...
export default defineEventHandler(async (event) => {
  try {
    ...
    // Read all files from incoming directory
    const files = await fileStorage.readIncomingFiles(processorId)
    ...
    throw createError({
      statusCode: 500,
      statusMessage: error instanceof Error ? error.message : 'Failed to read files'
    })
  }
}) 